...
func NewRPCStatsInterceptor(timeSource util.TimeSource, prefix string, mf MetricFactory) *RPCStatsInterceptor {
	interceptor := RPCStatsInterceptor{
		...
		ReqCount:          mf.NewCounter(prefixedName(prefix, reqCountName), "Number of requests", []string{methodName}),
		ReqSuccessCount:   mf.NewCounter(prefixedName(prefix, reqSuccessCountName), "Number of successful requests", []string{methodName}),
		ReqSuccessLatency: mf.NewHistogram(prefixedName(prefix, reqSuccessLatencyName), "Latency of successful requests", []string{methodName}),
		ReqErrorCount:     mf.NewCounter(prefixedName(prefix, reqErrorCountName), "Number of errored requests", []string{methodName}),
		ReqErrorLatency:   mf.NewHistogram(prefixedName(prefix, reqErrorLatencyName), "Latency of errored requests", []string{methodName}),
	}
	return &interceptor
}
...
func (r *RPCStatsInterceptor) Interceptor() grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
		...
		if err != nil {
			...
			r.ReqSuccessLatency.Observe(float64(latency.Nanoseconds()/int64(time.Millisecond)), labels...)
		}
